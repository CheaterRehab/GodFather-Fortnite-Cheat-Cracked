...
NTSTATUS HookControl(PDEVICE_OBJECT device, PIRP irp) {
	...
	NTSTATUS status = ps lookup process id ((HANDLE)SharedPid, &process);
	if (NT_SUCCESS(status) && process) {
		retry = false;
		REQUEST_DATA data;
		SIZE_T outSize = 0; //need to, if not == crash!
		if (NT_SUCCESS(Mm copyvirtualmemory(process, (void*)SharedBuffer, PsGetCurrentProcess(), &data, (SIZE_T)sizeof(REQUEST_DATA), KernelMode, &outSize))) {
...
INT64 driver_main() {
	// If you load the godfather cheat this shows up...
	// Wonder what UC driver means.
	// print("[!] UC-Driver start loading!");
	print("[!] God Father pasta start loading!!!");
	...
	if (DriverObject) {
		*(PVOID*)&OriginalPtr = InterlockedExchangePointer((void**)&DriverObject->MajorFunction[IRP_MJ_FLUSH_BUFFERS], HookControl);
		...
		print("[+] Driver Loaded! Get to pasting");
		return 0;
	}
	return 1;
}
